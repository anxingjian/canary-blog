...
// Warm Wave — fullscreen piece, link out
function WarmWaveLink() {
  const canvasRef = useRef<HTMLCanvasElement>(null);
  ...
  useEffect(() => {
    ...
    const WARM_DARK = [45, 30, 20];
    const COLORS = [AMBER, SIENNA, BRICK, WARM_GOLD, WARM_DARK];
    ...
    interface FP { x: number; y: number; px: number; py: number; s: number; l: number; ml: number; ci: number; }
    const particles: FP[] = [];
    for (let i = 0; i < 800; i++) {
      const x = Math.random() * W;
      const y = H * 0.1 + Math.random() * H * 0.6;
      particles.push({ x, y, px: x, py: y, s: 0.5 + Math.random() * 1.5, l: Math.random() * 150, ml: 80 + Math.random() * 150, ci: Math.floor(Math.random() * COLORS.length) });
    }
    ...
    function animate() {
      ...
      for (const p of particles) {
        p.l++;
        if (p.l > p.ml || p.x < -10 || p.x > W + 10 || p.y < -10 || p.y > H + 10) {
          p.x = Math.random() * W; p.y = H * 0.1 + Math.random() * H * 0.6;
          p.px = p.x; p.py = p.y; p.l = 0; p.ci = Math.floor(Math.random() * COLORS.length);
          continue;
        }
        p.px = p.x; p.py = p.y;
        ...
        const dist = Math.sqrt(dx * dx + dy * dy);
        const curl = Math.max(0, 1 - dist / (W * 0.35));
        const curlA = Math.atan2(dy, dx) + Math.PI * 0.5;
        const baseA = Math.PI * 0.15 + noise2d(nx * 3, ny * 3, time * 0.3) * 0.5;
        const angle = baseA * (1 - curl * 0.8) + curlA * curl * 0.8 + noise2d(nx * 6, ny * 6, time * 0.5) * 0.4;
        p.x += Math.cos(angle) * p.s;
        p.y += Math.sin(angle) * p.s;
        const lf = p.l / p.ml;
        const alpha = (lf < 0.1 ? lf / 0.1 : lf > 0.8 ? (1 - lf) / 0.2 : 1) * 0.25;
        const c = COLORS[p.ci];
        ctx!.beginPath();
  ...
  return (
    <a href="/canary-blog/arts/warm-wave" style={{ display: "block", width: "100%", aspectRatio: "1/1", background: "#0a0a0a", borderRadius: 6, overflow: "hidden", cursor: "pointer", textDecoration: "none" }}>
      <canvas ref={canvasRef} width={800} height={800} style={{ display: "block", width: "100%", height: "100%" }} />
    </a>
  );
}
...
const PIECES = [
  {
    id: "warm-wave",
    title: "暖浪",
    subtitle: "Warm Wave — after Hokusai",
    description: "如果神奈川冲浪里发生在夕阳下。普鲁士蓝变成琥珀，白色浪花变成暖金。Flow field 粒子沿着巨浪的弧线卷曲，不是复制一幅画，是用生成艺术重新感受那股力量。色彩练习：用暖色重新诠释冷色名画。",
    medium: "Canvas API · Flow Field · Generative",
    date: "2026.03.16",
